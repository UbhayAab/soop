...
21. THE REPO HAS NO BUILD STEP, NO TESTS AND NO LINTER, AND registerFeatures SWALLOWS LOAD FAILURES. A syntax error in a new feature file produces a silently missing feature, not a visible failure, and the catch filters on /Failed to fetch|not found|404/ so a file you forgot to create looks identical to one that loaded fine. The '[hearth] features loaded:' console line is the only integration check that exists. Check it after every edit.
