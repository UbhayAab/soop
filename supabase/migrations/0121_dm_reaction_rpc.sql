-- RENUMBERED from 0119. Two branches fixed the same three reports at the same
-- time and both took the numbers 0119 and 0120; 0119_direct_calls.sql and
-- 0120_dm_reactions_and_admin_badges.sql are the ones that were applied to the
-- live project first, so this file and 0122 moved up rather than being dropped.
--
-- Everything below is additive on top of 0120 and safe to run after it. The
-- table it creates already exists with exactly this shape (`if not exists` is a
-- no-op); what is genuinely new here is toggle_dm_reaction, kept because it
-- carries two rules 0120's routing branch did not: a deleted message cannot be
-- reacted to, and somebody who has blocked you does not receive your reactions.
-- 0123 folds both of those into public.toggle_reaction, which is what the client
-- actually calls. This one stays as the direct, single-purpose entry point.
--
